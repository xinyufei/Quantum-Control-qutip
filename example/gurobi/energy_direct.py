...
u = energy.addVars(N, T, lb=0, ub=1, vtype=gp.GRB.CONTINUOUS)
...
#     (matrix_exp[j].real @ real_op[k][:, col] - matrix_exp[j].imag @ imag_op[k][:, col]) * u[j, k]
...
for j in range(N):
    for k in range(T):
        ...
        next_real_op_list = real_op[k + 1].tolist()
        next_imag_op_list = imag_op[k + 1].tolist()
        energy.addConstrs(next_real_op_list[row][col] - sum(
            u[j, k] * sum(matrix_exp[j].real[row][temp] * real_op_list[temp][col] -
                          matrix_exp[j].imag[row][temp] * imag_op_list[temp][col] for temp in range(2 ** n))
            for j in range(N)) == 0 for row in range(2 ** n) for col in range(2 ** n))
        energy.addConstrs(next_imag_op_list[row][col] - sum(
            u[j, k] * sum(matrix_exp[j].real[row][temp] * imag_op_list[temp][col] +
                          matrix_exp[j].imag[row][temp] * real_op_list[temp][col] for temp in range(2 ** n))
            for j in range(N)) == 0 for row in range(2 ** n) for col in range(2 ** n))
...
# energy_lp = energy.relax()
energy.Params.NonConvex = 2
energy.Params.TimeLimit = 3600
energy.Params.LogFile = "../output/gurobi/EnergyDirectLR2_evotime2.0_n_ts40_instance0_tl3600.log"
energy.optimize()
...
for j in range(N):
    for k in range(T):
        u_val[k, j] = u[j, k].x
np.savetxt("../control/gurobi/EnergyDirectLR2_evotime2.0_n_ts40_instance0_tl3600.csv", u_val, delimiter=',')
