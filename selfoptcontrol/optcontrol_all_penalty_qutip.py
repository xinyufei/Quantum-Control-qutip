...
class Optcontrol_General_Qutip():
    """
    ...
    def build_optimizer(self, H_d, H_c, X_0, X_targ, n_ts, evo_time, amp_lbound=0, amp_ubound=1, ops_max_amp=1,
        ...
        optim = cpo.create_pulse_optimizer(self.H_d_qobj,
                                           ...
                                           self.X_0_qobj, self.X_targ_qobj,
    ...
    def _compute_err(self, *args):
        ...
        # print(1 - fid)
        return 1 - fid + self.rho / 2 * norm + penalized
        # return 1 - fid
    ...
    def _fprime(self, *args):
        ...
        return fid_grad + norm_grad.flatten() + penalized_grad.flatten()
    ...
    def _minimize_u(self):
        ...
        result = self.qutip_optimizer.run_optimization_all_penalty()
        self.u = result.final_amps
        self.cur_obj = result.fid_err + self.rho / 2 * self.compute_norm(self.u) + self.compute_penalty(self.u)
    ...
    def optimize_admm(self):
        ...
        print("Final penalized l2 error {}".format(self.compute_penalty(self.u)), file=report)
        ...
        print("Final l2 value {}".format(
            sum(np.power(sum(self.u[t, j] for j in range(self.n_ctrls)) - self.max_controllers, 2)
                for t in range(self.n_ts))), file=report)
