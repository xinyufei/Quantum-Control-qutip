...
class Optcontrol_Exact_Qutip():
    ...
    def _fprime(self, *args):
        ...
        for t in range(self.n_ts):
            for j in range(self.n_ctrls):
                grad_temp = expm_frechet(-1j * H[t] * delta_t, -1j * self.ops_max_amp[j] * self.H_c[j] * delta_t,
                                         compute_expm=False)
    ...
    def _minimize_u(self):
        ...
        cons = [{'type': 'eq',
                 'fun': lambda x: x[k * self.n_ctrls:(k + 1) * self.n_ctrls].sum() - self.max_controllers}
                for k in range(self.n_ts)]
        # print(cons)
        # cons = [{'type': 'eq',
        #          'fun': lambda x: x.sum() - self.max_controllers} ]
        # print(cons)
        # result = optim.run_optimization_sum_penalty()
        result = optim.run_optimization_with_cons(cons=cons)
        # print(result[2]['grad'])
        ...
        self.result = result
        # print(self.compute_fid(self.evolution(
        #     self.u + 0.1 * optim.fid_err_grad_wrapper_sum_penalty(self.u).reshape((self.n_ts, self.n_ctrls)))))
        # print(optim.fid_err_func_wrapper_sum_penalty(self.u.reshape(-1)))
        # print(optim.fid_err_func_wrapper_sum_penalty(
        #     self.u.reshape(-1) + 0.1 * optim.fid_err_grad_wrapper_sum_penalty(self.u)))
